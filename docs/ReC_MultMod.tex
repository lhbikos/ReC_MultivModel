...
## AppData/Local/Temp/Rtmpon33FQ/Recent Course Eval.csv', reason 'Permission
## denied'
\end{verbatim}
...
CAPTURING RESULTS FOR WRITING IT UP: Data screening suggested that 92 individuals opened the survey link.
...
CAPTURING RESULTS FOR WRITING IT UP: Data screening suggested that 92 individuals opened the survey link. Of those, 74, granted consent and proceeded into the survey items.
...
CAPTURING RESULTS FOR WRITING IT UP: Data screening suggested that 92 individuals opened the survey link. Of those, 74, granted consent and proceeded into the survey items. A further inclusion criteria was that the course was taught in the U.S; 65 met this criteria.
...
Data screening suggested that 92 individuals opened the survey link. Of those, 74 granted consent and proceeded to the survey items. A further inclusion criteria was that the course was taught in the U.S; 65 met this criteria.
...
\begin{verbatim}
## 'data.frame':    65 obs. of  25 variables:
...
## X1    1 65 8.25 23.22      0    1.72   0   0 100   100 2.92     7.43 2.88
\end{verbatim}
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 100\%.
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 100\%. Across the dataset, 4.11\% of cells had missing data and 87.10\% of cases had nonmissing data.
...
\begin{verbatim}
##    vars  n mean    sd median trimmed mad min   max range skew kurtosis   se
## X1    1 62 4.03 12.69      0    0.33   0   0 66.67 66.67  3.3    10.76 1.61
\end{verbatim}
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across the 62 cases for which the scoring protocol was applied, missingness ranged from 0\% to 67\%.
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across the 62 cases for which the scoring protocol was applied, missingness ranged from 0\% to 67\%. After eliminating cases with greater than 20\% missing, the dataset analyzed included 57 cases.
...
CUMULATIVE CAPTURE FOR WRITING IT UP: Across the 62 cases for which the scoring protocol was applied, missingness ranged from 0\% to 67\%. After eliminating cases with greater than 20\% missing, the dataset analyzed included 57 cases. In this dataset we had 0.58\% missing across the df; 96.49\% of the rows had nonmissing data.
...
\includegraphics{ReC_MultMod_files/figure-latex/unnamed-chunk-43-1.pdf}
...
Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 100\%. Across the dataset, 4.11\% of cells had missing data and 87.10\% of cases had nonmissing data. At this stage in the analysis, we allowed all cases with less than 90\% missing to continue to the scoring stage. Guided by Parent's \citeyearpar{parent_handling_2013} AIA approach, scales with three items were scored if at least two items were non-missing; the scale with four items was scored if it at least three non-missing items; and the scale with six items was scored if it had at least five non-missing items.

Across the 62 cases for which the scoring protocol was applied, missingness ranged from 0\% to 67\%. After eliminating cases with greater than 20\% missing, the dataset analyzed included 57 cases. In this dataset we had 0.58\% missing across the df; 96.49\% of the rows had nonmissing data.
...
##       0.96      0.96    0.94      0.89  24 0.0088    4 1.5     0.89
## 
...
## Belong_1      0.94      0.94    0.89      0.89  17    0.014    NA  0.89
## Belong_2      0.93      0.93    0.88      0.88  14    0.017    NA  0.88
## Belong_3      0.95      0.95    0.90      0.90  18    0.014    NA  0.90
## 
##  Item statistics 
##           n raw.r std.r r.cor r.drop mean  sd
## Belong_1 60  0.96  0.96  0.93   0.91  4.1 1.5
## Belong_2 61  0.97  0.97  0.95   0.93  4.1 1.6
## Belong_3 60  0.96  0.96  0.93   0.91  3.8 1.6
## 
## Non missing response frequency for each item
##             1    2    3    4    5    6    7 miss
## Belong_1 0.02 0.15 0.23 0.18 0.22 0.15 0.05 0.03
## Belong_2 0.03 0.15 0.20 0.23 0.15 0.20 0.05 0.02
## Belong_3 0.05 0.20 0.17 0.23 0.20 0.10 0.05 0.03
\end{verbatim}

CAPTURE FOR THE APA STYLE WRITE-UP(since these are generally placed in the description of each measure in the Method section, I am not doing a cumulative capture): Cronbach's alpha for the belonging scale was 0.960.
...
## 
##   raw_alpha std.alpha G6(smc) average_r S/N   ase mean  sd median_r
##       0.87      0.88    0.88      0.56 7.5 0.025  2.5 1.1     0.51
## 
##  lower alpha upper     95% confidence boundaries
## 0.82 0.87 0.92 
## 
##  Reliability if an item is dropped:
##         raw_alpha std.alpha G6(smc) average_r S/N alpha se var.r med.r
## rBlst_1      0.88      0.89    0.89      0.61 7.8    0.025 0.017  0.62
## Blst_2       0.87      0.87    0.87      0.58 7.0    0.027 0.020  0.55
## Blst_3       0.86      0.88    0.87      0.59 7.1    0.028 0.019  0.55
## Blst_4       0.83      0.85    0.84      0.52 5.5    0.034 0.019  0.47
## Blst_5       0.83      0.84    0.83      0.51 5.2    0.035 0.015  0.49
## Blst_6       0.84      0.85    0.83      0.52 5.5    0.033 0.016  0.51
## 
##  Item statistics 
##          n raw.r std.r r.cor r.drop mean  sd
## rBlst_1 56  0.70  0.68  0.57   0.54  3.5 1.6
## Blst_2  60  0.77  0.74  0.67   0.62  2.9 1.7
## Blst_3  59  0.71  0.73  0.65   0.59  2.1 1.3
## Blst_4  58  0.86  0.86  0.84   0.79  2.5 1.3
## Blst_5  59  0.89  0.89  0.89   0.83  2.1 1.3
## Blst_6  59  0.83  0.86  0.85   0.77  2.2 1.3
## 
## Non missing response frequency for each item
##            1    2    3    4    5    6    7 miss
## rBlst_1 0.09 0.23 0.21 0.23 0.09 0.11 0.04 0.10
## Blst_2  0.35 0.15 0.08 0.18 0.17 0.07 0.00 0.03
## Blst_3  0.42 0.34 0.07 0.12 0.03 0.02 0.00 0.05
## Blst_4  0.26 0.34 0.16 0.17 0.05 0.00 0.02 0.06
## Blst_5  0.44 0.31 0.05 0.15 0.05 0.00 0.00 0.05
## Blst_6  0.36 0.36 0.12 0.08 0.07 0.02 0.00 0.05
\end{verbatim}

CAPTURE FOR THE APA STYLE WRITE-UP (since these are generally placed in the description of each measure in the Method section, I am not doing a cumulative capture): Cronbach's alpha for the campus climate scale was 0.872. Since this value is \(\geq\) .80, it is within the realm of acceptability. Let's go ahead, though, and look at its subscales.
...
## 
##   raw_alpha std.alpha G6(smc) average_r S/N   ase mean  sd median_r
##       0.76      0.78    0.74      0.54 3.6 0.051  2.3 1.2     0.59
## 
##  lower alpha upper     95% confidence boundaries
## 0.66 0.76 0.86 
## 
##  Reliability if an item is dropped:
##        raw_alpha std.alpha G6(smc) average_r S/N alpha se var.r med.r
## Blst_3      0.78      0.80    0.67      0.67 4.0    0.052    NA  0.67
## Blst_2      0.74      0.74    0.59      0.59 2.9    0.065    NA  0.59
## Blst_5      0.52      0.54    0.37      0.37 1.2    0.114    NA  0.37
## 
##  Item statistics 
##         n raw.r std.r r.cor r.drop mean  sd
## Blst_3 59  0.75  0.78  0.61   0.51  2.1 1.3
## Blst_2 60  0.86  0.82  0.69   0.59  2.9 1.7
## Blst_5 59  0.90  0.90  0.85   0.76  2.1 1.3
## 
## Non missing response frequency for each item
##           1    2    3    4    5    6 miss
## Blst_3 0.42 0.34 0.07 0.12 0.03 0.02 0.05
## Blst_2 0.35 0.15 0.08 0.18 0.17 0.07 0.03
## Blst_5 0.44 0.31 0.05 0.15 0.05 0.00 0.05
\end{verbatim}

Cronbach's alpha for the campus climate stigma subscale was 0.764.
...
## 
##   raw_alpha std.alpha G6(smc) average_r S/N   ase mean  sd median_r
##       0.79      0.81    0.76      0.58 4.1 0.047  2.7 1.2     0.51
## 
##  lower alpha upper     95% confidence boundaries
## 0.7 0.79 0.89 
## 
##  Reliability if an item is dropped:
##         raw_alpha std.alpha G6(smc) average_r S/N alpha se var.r med.r
## rBlst_1      0.86      0.86    0.75      0.75 6.1    0.036    NA  0.75
## Blst_4       0.63      0.64    0.47      0.47 1.8    0.091    NA  0.47
## Blst_6       0.67      0.68    0.51      0.51 2.1    0.081    NA  0.51
## 
##  Item statistics 
##          n raw.r std.r r.cor r.drop mean  sd
## rBlst_1 56  0.81  0.78  0.57   0.53  3.5 1.6
## Blst_4  58  0.88  0.89  0.84   0.72  2.5 1.3
## Blst_6  59  0.85  0.87  0.81   0.69  2.2 1.3
## 
## Non missing response frequency for each item
##            1    2    3    4    5    6    7 miss
## rBlst_1 0.09 0.23 0.21 0.23 0.09 0.11 0.04 0.10
...
Cronbach's alpha for the campus climate responsiveness subscale was 0.793. Between the two subscales, it looks as if the responsivenes subscale is more internally consistent.
...
## iBIPOC_pr    1 60 0.35 0.39   0.25    0.31 0.37   0  1.00  1.00 0.63    -1.15
## cmBlack      2 62 7.66 7.61   5.00    6.70 7.41   0 29.00 29.00 0.92    -0.06
## Belonging    3 60 4.01 1.50   4.00    4.00 1.48   1  7.00  6.00 0.05    -0.93
## ClimateBL    4 57 2.50 1.12   2.33    2.44 1.23   1  5.67  4.67 0.49    -0.45
##             se
## iBIPOC_pr 0.05
## cmBlack   0.97
## Belonging 0.19
...
## W = 0.94942, p-value = 0.01851
\end{verbatim}

CUMULATIVE CAPTURE FOR THE APA STYLE WRITE-UP: Regarding the distributional characteristics of the data, skew and kurtosis values of the variables fell below the values of 3 (skew) and 8 to 20 (kurtosis) that Kline suggests are concerning \citeyearpar{kline_principles_2016}. Results of the Shapiro-Wilk test of normality indicate that our variables assessing the proportion of classmates who are Black (\(W\) = 0.876, \(p\) = 0.0000) and the proportion of BIPOC instructional staff(\(W\) = 0.789, \(p\) = 0.0000) are statistically significantly different than a normal distribution. The scales assessing the respondent's belonging (\(W\) = 0.971, \(p\) = 0.1610) and the respondent's perception of campus climate for Black students (\(W\) = 0.949, \(p\) = 0.0185) did not differ differently from a normal distribution.
...
\includegraphics{ReC_MultMod_files/figure-latex/unnamed-chunk-55-1.pdf}
...
\includegraphics{ReC_MultMod_files/figure-latex/unnamed-chunk-56-1.pdf}
...
\begin{verbatim}
##    vars  n mean  sd median trimmed  mad  min   max range skew kurtosis   se
## X1    1 62  3.8 2.2   3.64    3.62 2.47 0.26 10.67 10.42 0.77     0.45 0.28
\end{verbatim}
...
## 1      2      2       3      6.33       2.33     3.33      2.83 3.1529026
## 2      4      1       2      4.67       1.67     4.00      2.83 1.1055607
## 3     NA      2      NA        NA       2.00     3.50        NA 0.2556646
## 4      2      2       6      3.33       3.33     2.00      2.67 0.2881807
## 5      1      1       2      4.00       1.33     1.00      1.17 3.6918977
## 6      1      2       3      5.33       2.33     1.00      1.67 5.2424055
##   MOutlier
## 1    FALSE
## 2    FALSE
## 3    FALSE
...
## -1.90325 -0.81374 -0.00349  0.77495  2.99229 
## 
## Coefficients:
##             Estimate Std. Error t value Pr(>|t|)    
## (Intercept)  2.94490    0.48519   6.070  1.6e-07 ***
## Belonging   -0.01788    0.10092  -0.177   0.8601    
## cmBlack     -0.01999    0.01920  -1.041   0.3028    
## iBIPOC_pr   -0.71144    0.38193  -1.863   0.0683 .  
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 1.099 on 51 degrees of freedom
##   (7 observations deleted due to missingness)
## Multiple R-squared:  0.08627,    Adjusted R-squared:  0.03252 
## F-statistic: 1.605 on 3 and 51 DF,  p-value: 0.1996
\end{verbatim}
...
Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging (\(B\) = -0.018, \(p\) = 0.860) nor the proportion of BIPOC instructional staff (\(B\) = -0.711, \(p\) = 0.068), nor the proportion of Black classmates (\(B\) = -0.020, \(p\) = 0.303) led to statistically significant changes in perceptions of campus climate for Black students. Although it accounted for 8.63\% of the variance, the overall model was not statistically significant. Results are presented in Table 2.
...
##                                                             
##   2. cmBlack   7.66 7.61 .03                                
##                          [-.22, .28]                        
##                                                             
##   3. Belonging 4.01 1.50 .03         -.11                   
##                          [-.23, .29] [-.36, .15]            
##                                                             
##   4. ClimateBL 2.50 1.12 -.26        -.16        -.05       
##                          [-.49, .01] [-.40, .10] [-.30, .22]
##                                                             
## 
...
##  (Intercept) 2.94**  [1.97, 3.92]                                         
##    Belonging  -0.02 [-0.22, 0.18] -0.02 [-0.29, 0.25] .00 [-.01, .01] -.01
##      cmBlack  -0.02 [-0.06, 0.02] -0.14 [-0.41, 0.13] .02 [-.05, .09] -.15
##    iBIPOC_pr  -0.71 [-1.48, 0.06] -0.25 [-0.52, 0.02] .06 [-.06, .18] -.26
##                                                                           
##                                                                           
##                                                                           
##              Fit
...
## 0.13846154 0.10769231 0.09230769 0.09230769 0.09230769 0.07692308 0.07692308 
##     Blst_2   Belong_2    cEval_1   cEval_19  iBIPOC_pr    cmBlack   cEval_13 
## 0.07692308 0.06153846 0.06153846 0.06153846 0.04615385 0.04615385 0.04615385
\end{verbatim}
...
##  1 (Intercept)  2.98       0.473     6.31   0.0000000449    61
##  2 Belonging   -0.0151     0.0971   -0.155  0.877           61
##  3 cmBlack     -0.0213     0.0191   -1.12   0.269           61
##  4 iBIPOC_pr   -0.635      0.365    -1.74   0.0872          61
##  5 (Intercept)  2.98       0.456     6.53   0.0000000192    61
##  6 Belonging   -0.00946    0.0955   -0.0990 0.921           61
##  7 cmBlack     -0.0202     0.0187   -1.08   0.284           61
##  8 iBIPOC_pr   -0.810      0.363    -2.23   0.0295          61
##  9 (Intercept)  3.02       0.468     6.44   0.0000000268    61
## 10 Belonging   -0.0333     0.0956   -0.349  0.728           61
## 11 cmBlack     -0.0205     0.0187   -1.10   0.277           61
## 12 iBIPOC_pr   -0.589      0.353    -1.67   0.100           61
## 13 (Intercept)  3.00       0.465     6.45   0.0000000260    61
## 14 Belonging   -0.0125     0.0964   -0.130  0.897           61
## 15 cmBlack     -0.0206     0.0189   -1.09   0.282           61
## 16 iBIPOC_pr   -0.758      0.372    -2.04   0.0460          61
## 17 (Intercept)  3.06       0.458     6.68   0.0000000110    61
## 18 Belonging   -0.0256     0.0949   -0.270  0.788           61
## 19 cmBlack     -0.0219     0.0187   -1.17   0.246           61
## 20 iBIPOC_pr   -0.747      0.368    -2.03   0.0467          61
\end{verbatim}
...
## 1 (Intercept) 5  3.00735694 0.2154077988 1.005386e-03 0.2166142615    57
## 2   Belonging 5 -0.01920803 0.0091942123 9.935813e-05 0.0093134420    57
## 3     cmBlack 5 -0.02089314 0.0003538947 4.763674e-07 0.0003544664    57
## 4   iBIPOC_pr 5 -0.70790064 0.1324881894 8.469216e-03 0.1426512482    57
##         df         riv      lambda        fmi
## 1 54.76984 0.005600831 0.005569636 0.03999696
## 2 54.27366 0.012967914 0.012801900 0.04727492
## 3 55.00917 0.001615285 0.001612680 0.03603438
## 4 48.05398 0.076709168 0.071244093 0.10762738
\end{verbatim}
...
## 1 (Intercept)  3.00735694 0.46541837  6.4616206 54.76984 2.911177e-08
## 2   Belonging -0.01920803 0.09650618 -0.1990342 54.27366 8.429798e-01
## 3     cmBlack -0.02089314 0.01882728 -1.1097272 55.00917 2.719443e-01
## 4   iBIPOC_pr -0.70790064 0.37769200 -1.8742802 48.05398 6.697797e-02
\end{verbatim}
...
Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging (\(B\) = -0.019, \(p\) = 0.843) nor the proportion of BIPOC instructional staff (\(B\) = -0.708, \(p\) = 0.067) led to statistically significant changes in perceptions of campus climate for Black students. Results did suggest a statistically significant negative effect of the proportion of Black classmates on the campus climate for Black students. Specifically, as the proportion of Black students increases, there is less stigmatization and hostility on the campus (\(B\) = -0.021, \(p\) = 0.272). Results are presented in Table X.
...
Data screening suggested that 92 individuals opened the survey link. Of those, 74 granted consent and proceeded to the survey items. A further inclusion criteria was that the course was taught in the U.S; 65 met this criteria. Across cases that were deemed eligible on the basis of the inclusion/exclusion criteria, missingness ranged from 0\% to 100\%. Across the dataset, 4.11\% of cells had missing data and 87.10\% of cases had nonmissing data. At this stage in the analysis, we allowed all cases with fewer than 50\% missing to be included the multiple imputation \citep{katitas_getting_2019}.
...
Results of a multiple regression predicting the respondents' perceptions of campus climate for Black students indicated that neither contributions of the respondents' personal belonging (\(B\) = -0.018, \(p\) = 0.860) nor the proportion of BIPOC instructional staff (\(B\) = -0.711, \(p\) = 0.068) led to statistically significant changes in perceptions of campus climate for Black students. Results did suggest a statistically significant negative effect of the proportion of Black classmates on the campus climate for Black students. Specifically, as the proportion of Black students increases, there is less stigmatization and hostility on the campus (\(B\) = -0.020, \(p\) = 0.303). Although it accounted for 8.63\% of the variance, the overall model was not statistically significant. Results are presented in Table 2.
## 156  0.55409136 2.7994068 4.8666557  3.8620707 1.337963  1.57531900
\end{verbatim}
...
\includegraphics{ReC_MultMod_files/figure-latex/unnamed-chunk-81-1.pdf}
...
\addcontentsline{toc}{chapter}{MODERATION}
...
\includegraphics{ReC_MultMod_files/figure-latex/drawing a line of best fit-1.pdf}
...
\includegraphics{ReC_MultMod_files/figure-latex/pairs panels for Kim et al-1.pdf}
...
\includegraphics{ReC_MultMod_files/figure-latex/Plot Kim SimpMod-1.pdf}
...
\includegraphics{ReC_MultMod_files/figure-latex/Plot Kim SimpMod-2.pdf}
...
\NormalTok{KimSimpModMLE }\OtherTok{\textless{}{-}} \StringTok{\textquotesingle{}}
...
\StringTok{\textquotesingle{}}
\NormalTok{kMLE\_fit }\OtherTok{\textless{}{-}} \FunctionTok{sem}\NormalTok{(KimSimpModMLE, }\AttributeTok{data =}\NormalTok{ Kim\_df, }\AttributeTok{missing =} \StringTok{\textquotesingle{}fiml\textquotesingle{}}\NormalTok{, }\AttributeTok{se =} \StringTok{"bootstrap"}\NormalTok{, }\AttributeTok{bootstrap =} \DecValTok{1000}\NormalTok{)}
\NormalTok{k1summary }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(kMLE\_fit, }\AttributeTok{standardized =} \ConstantTok{TRUE}\NormalTok{, }\AttributeTok{rsq=}\NormalTok{T, }\AttributeTok{ci=}\ConstantTok{TRUE}\NormalTok{)    }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{k1ParamEsts }\OtherTok{\textless{}{-}} \FunctionTok{parameterEstimates}\NormalTok{(kMLE\_fit, }\AttributeTok{boot.ci.type =} \StringTok{"bca.simple"}\NormalTok{, }\AttributeTok{standardized=}\ConstantTok{TRUE}\NormalTok{)}
\NormalTok{k1summary}
\end{Highlighting}
\end{Shaded}
...
\includegraphics{ReC_MultMod_files/figure-latex/semPlot of simple mod-1.pdf}
...
\begin{Highlighting}[]
\FunctionTok{library}\NormalTok{(lavaan)}
\CommentTok{\#set.seed(210501)}
\CommentTok{\#MLEmodel \textless{}{-} \textquotesingle{}}
    \CommentTok{\#Y \textasciitilde{} b1*X + b2*W + b3*X:W}
    \CommentTok{\#intercept (constant) of Y}
    \CommentTok{\#Y \textasciitilde{} Y.mean*1}
    \CommentTok{\#mean of W  for use in simple slopes}
    \CommentTok{\#W \textasciitilde{} W.mean*1}
    \CommentTok{\#variance of W for use in simple slopes}
    \CommentTok{\#W \textasciitilde{}\textasciitilde{} W .var*W}

    \CommentTok{\#simple slopes}
    \CommentTok{\#SD.below := b1 + b3*(W.mean {-} sqrt(W.var))}
    \CommentTok{\#mean := b1 + b3*(W.mean)}
    \CommentTok{\#SD.above := b1 + b3*(W.mean + sqrt(W.var))}
\CommentTok{\#\textquotesingle{}}
\CommentTok{\#MLEmod\_fit \textless{}{-} sem(MLEmodel, data = my\_df, missing = \textquotesingle{}fiml\textquotesingle{}, se = "bootstrap", bootstrap = 1000)}
\CommentTok{\#MLEmod\_fit\_summary \textless{}{-} summary(MLEmod\_fit, standardized = TRUE, rsq=T, ci=TRUE)    }
\CommentTok{\#MLEmodParamEsts \textless{}{-} parameterEstimates(MLEmod\_fit, boot.ci.type = "bca.simple", standardized=TRUE)}
\CommentTok{\#MLEmod\_fit\_summary}
\CommentTok{\#MLEmodParamEsts}
\end{Highlighting}
\end{Shaded}
